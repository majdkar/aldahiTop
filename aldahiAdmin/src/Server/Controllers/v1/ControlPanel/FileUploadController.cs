...
namespace FirstCall.Api.Controllers
{
    public class FileUploadController : ApiControllerBase
    {
        ...
        public async Task<IActionResult> Upload([FromForm] IFormFile file, int fileLocation, int uploadType)
        {
            try
            {
                var requestUploadType = (Enums.UploadType)uploadType;
                if (!CheckFileExtension(file, requestUploadType))
                {
                    return BadRequest($"The File does not have an extension or it is not image. " +
                        $"The Expected extension is .jpg/.png/.bmp/.webp");
                }

                if (requestUploadType != Enums.UploadType.Video)
                {
        ...
        private bool CheckFileExtension(IFormFile file, Enums.UploadType type)
        {
            var fileNameExtension = file.FileName.Split(".").LastOrDefault();

            string[] extensions = new string[] { };
            if (type == Enums.UploadType.Image)
                extensions = new string[] { "jpg", "bmp", "png", "jpeg", "webp" };
            if (type == Enums.UploadType.File)
                extensions = new string[] { "txt", "docx", "pdf", "xlsx" };
            if (type == Enums.UploadType.Video)
