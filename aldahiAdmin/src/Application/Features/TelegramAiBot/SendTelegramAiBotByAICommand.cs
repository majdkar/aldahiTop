...
namespace FirstCall.Application.Features.ContactUs.Commands.AddEdit
{
    ...
    internal class SendTelegramAiBotByAICommandHandler : IRequestHandler<SendTelegramAiBotByAICommand, Result<int>>
    {
        ...
        private readonly SemaphoreSlim _semaphore = new SemaphoreSlim(5); // حد أقصى 5 طلبات متزامنة
        ...
        private void EnqueueRequest(Func<Task> work)
        {
            lock (_queue)
            {
                _queue.Enqueue(work);
                if (_queue.Count == 1)
                    _ = ProcessQueue(); // تشغيل الطابور إذا لم يكن شغال
            }
        }
        ...
        private async Task ProcessQueue()
        {
            while (true)
            {
                ...
                {
                    // يمكن تسجيل الخطأ هنا إذا أحببت
                }
                finally
                {
                    _semaphore.Release();
        ...
        private async Task<string> ExecuteOpenAiRequestAsync(StringContent requestContent)
        {
            while (true)
            {
                ...
                if (response.StatusCode == System.Net.HttpStatusCode.TooManyRequests)
                {
                    ...
                    continue; // أعد المحاولة بعد التأخير
                }
        ...
        //                            Warehouses(Id, NameAr, NameEn, IsDeleted)
        ...
        //                            Product(Id, NameAr, NameEn, Sizes, ProductImageUrl,  Code, WarehousesId, Price, [Order], Qty, Colors, PackageNumber, IsDeleted, KindId, SeasonId, ProductCategoryId)
        ...
        //                            Product ↔ Warehouses → Product.WarehousesId = Warehouses.Id
        ...
        //                             المستودع, المخزن, Warehouses → Warehouses table.
        ...
        //                    if (kv.Key == "GroupId") return null; // لا نعرض الرابط نفسه في النص
        //                    if (kv.Key == "SeasonId") return null; // لا نعرض الرابط نفسه في النص
        //                    if (kv.Key == "CategoryId") return null; // لا نعرض الرابط نفسه في النص
        //                    if (kv.Key == "ProductCategoryId") return null; // لا نعرض الرابط نفسه في النص
        //                    if (kv.Key == "WarehousesId") return null; // لا نعرض الرابط نفسه في النص
        ...
        //                        "KindNameAr" => "🧩 النوع",
        //                        "SeasonNameAr" => "🧩 الموسم",
        //                        "ProducCategoryNameAr" => "🗂️ الصنف",
        //                        "GroupNameAr" => "🗂️ المجموعة",
        //                        _ => $"🔸 {kv.Key}"
        //                    };
        ...
        //                {
        //                    photoUrl = "https://admin.aldahitop.com/" + photoUrl;
        ...
        //    catch (Exception ex)
        //    {
        //        return await Result<int>.FailAsync("Fail");
        //    }
        //    return await Result<int>.SuccessAsync("Ok");
...
Request: {command.naturalLanguageRequest}";
                ...
                // أدخل طلب OpenAI في الطابور
                EnqueueRequest(async () =>
                {
                    var jsonResponse = await ExecuteOpenAiRequestAsync(requestContent);
                ...
                await tcs.Task; // انتظر حتى ينتهي طلب OpenAI
