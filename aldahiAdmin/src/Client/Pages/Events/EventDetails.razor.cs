...
namespace FirstCall.Client.Pages.Events
{
    public partial class EventDetails
    {
        ...
        private async Task SaveAsync()
        {
            ...
            var generatedImageName = await UploadFile(_images, imageUploadModel, (int)Enums.UploadType.Image);
            var fullImagePath = Path.Combine(Constants.UploadFolderName, Enums.FileLocation.EventsFiles.ToString(), generatedImageName);


            var generatedVideoName = await UploadFile(_videos, videoUploadModel, (int)Enums.UploadType.Video);
